...
public class CacheDiskLru implements BaseCache{


    private static final String TAG = "DiskLruImageCache";
    private DiskLruCache diskCache;
    private static CacheDiskLru instance;
    ...
    private CacheDiskLru(final Context context) {
        this(context, compressFormat, compressQuality);
    }

    //initer
    private CacheDiskLru(final Context context, final Bitmap.CompressFormat format, final int quality) {
        try {
            final File diskCacheDir = develop.godbeom.com.myapplication.util.Utils.getDiskCacheDir(context, CacheDiskLru.TAG);
            diskCache = DiskLruCache.open(diskCacheDir, APP_VERSION, VALUE_COUNT, CacheDiskLru.DISK_CACHE_SIZE);
    ...
    public static CacheDiskLru getInstance(final Context context) {
        if (instance == null) {
            instance = new CacheDiskLru(context);
        }
        return instance;
    }
