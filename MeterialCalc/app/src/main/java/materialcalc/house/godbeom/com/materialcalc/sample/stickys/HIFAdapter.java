...
public class HIFAdapter extends SectioningAdapter {
	...
	public StickyItemHolder onCreateItemViewHolder(ViewGroup parent, int itemUserType) {
		switch (itemUserType) {
			case ITEM_A:
				return new ItemA(inflateHolderView(mContext, R.layout.item_a,parent,false));
			case ITEM_B:
				return new ItemB(inflateHolderView(mContext, R.layout.item_b,parent,false));

			case ITEM_HORIZANTAL_LIST:
				return new ItemHorizantalList(inflateHolderView(mContext, R.layout.item_list_content,parent,false));
			case BANNER_A:
				return new ItemBannerA(inflateHolderView(mContext, R.layout.item_benner,parent,false));
	...
	public StickyHeaderHolder onCreateHeaderViewHolder(ViewGroup parent, int headerType) {
		switch (headerType) {
			case HEADER_A:
				return new HeaderA(inflateHolderView(mContext, R.layout.header_a,parent,false));
			case HEADER_B:
				return new HeaderB(inflateHolderView(mContext, R.layout.header_b,parent,false));
			default:
				return new HeaderB(inflateHolderView(mContext, R.layout.header_b,parent,false));
		}
	}
	...
	public StickyFooterHolder onCreateFooterViewHolder(ViewGroup parent, int footerType) {
		switch (footerType) {
			case FOOTER_A:
				return new FooterA(inflateHolderView(mContext, R.layout.footer_a,parent,false));
			case FOOTER_B:
				return new FooterB(inflateHolderView(mContext, R.layout.footer_b,parent,false));
			default:
				return new FooterB(inflateHolderView(mContext, R.layout.footer_b,parent,false));
		}
	}
	...
	private View inflateHolderView(Context context ,int viewId,ViewGroup parent,boolean attachToRoot){
